...
{
public:
    YOLONode() : Node("yolo_node"), processing_(false)
    {
        ...
        try {
            //dnn is deep neural netowrk
            //creates neural network
            net_ = cv::dnn::readNetFromDarknet(cfg, weights);
            //Specifices which computation engine should run network, currently using OpenCV"s own implmentaiton 
            net_.setPreferableBackend(cv::dnn::DNN_BACKEND_OPENCV);
            //Specifies what hardware to use for computations
            net_.setPreferableTarget(cv::dnn::DNN_TARGET_CPU);
            
            // Get output layer names
            // These layers produce final bounding boxes and class predicitons, need these names to extract network's predicitons
    ...
    void initialize()
    {
        ...
        image_publisher_ = it_->advertise("/camera/yolo_image", 1);  // Queue size 1, not 10
...
private:
    void imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
    {
        ...
        //Passing blob through neural network into Mat outs
